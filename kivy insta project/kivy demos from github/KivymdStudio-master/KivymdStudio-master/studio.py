...
class Studio(QObject):
    ...
    def richcolor(self, text):
        ...
        syntax = Syntax(text, "python", background_color="#1F1F20",
                        indent_guides=True, tab_size=4, theme='native')
        console.print(syntax)
        r = console.export_html(
            code_format="<pre>{code}</pre>", inline_styles=True)
    ...
    def run_command(self, cmd):
        ...
        executeur = subprocess.Popen(str(
            cmd), shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)
        sortie = str(executeur.stdout.read() +
                     executeur.stderr.read())[2:-3].split('\n')
        srt = '\n\r'.join(sortie)
    ...
    def run(self):
        ...
        engine.load(
            os.fspath(Path(__file__).resolve().parent / "qml/studio.qml"))
